...
const VoiceFinder: React.FC<VoiceFinderProps> = ({ voices, onRecommendation, onClose }) => {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Use the shared key manager to get a valid key from the potentially comma-separated string
      ...
      // If unauthorized, it might be due to an exhausted key, rotate for next try
      if (JSON.stringify(err).includes('429') || JSON.stringify(err).includes('quota')) {
         keyManager.jailCurrentKey(5000); 
      }
